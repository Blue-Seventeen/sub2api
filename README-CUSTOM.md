...
| 模型广场展示 | `/model-plaza?embedded=1` 从 `/admin/channels/pricing` 渠道定价读取官方价，实付价按官方价 × 生效倍率展示；支持分组自定义 `/v1/models` 白名单、用户有效订阅分组可见性、全部平台按平台聚合并选择最低倍率 | 本能力只影响模型广场展示，不改变真实中转、账号调度、余额扣费、订阅扣费、usage 记录或渠道定价保存语义 | `backend/internal/service/channel_plaza.go`, `frontend/src/components/modelPlaza/*`, `frontend/src/components/common/GroupSelector.vue` |
