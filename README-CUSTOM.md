...
|---|---|
| 当前主线 | `dev` |
| 当前 upstream 基线 | 已同步到 `v0.1.172`，`backend/cmd/server/VERSION` 已按 release 号对齐为 `0.1.172` |
...
| 模型广场展示 | `/model-plaza?embedded=1` 从 `/admin/channels/pricing` 渠道定价读取官方价，实付价按官方价 × 生效倍率展示；支持分组自定义 `/v1/models` 白名单、用户有效订阅分组可见性（仅生效期内展示）、全部平台按平台聚合并选择最低生效倍率对应模型 | 本能力只影响模型广场展示，不改变真实中转、账号调度、余额扣费、订阅扣费、usage 记录或渠道定价保存语义 | `backend/internal/service/channel_plaza.go`, `frontend/src/components/modelPlaza/*`, `frontend/src/components/common/GroupSelector.vue` |
...
- 当前 `codex/sync-v0.1.165` 已合入 upstream `v0.1.165`，并吸收 OpenAI Live schema/API/UI 字段、`session_id` usage 记录字段、ChatGPT/Claude/Ollama 兼容改进、公告前端增强、模型定价更新与邮箱别名去重迁移等更新。合并时必须继续保留本 fork 的 Promotion、CompatibleGateway、NewAPI-style/GLM 根 URL 兼容、Grok API Key + OAuth 单平台、多时段高峰倍率、用户专属统一倍率显示语义、订阅滚动与多订阅扣费、usage fallback、compact SSE 终止符加固和用户侧错误脱敏。
- v0.1.165 上线前防回归约束：OpenAI Live 只允许保留 additive schema、迁移、DTO 与前端兼容字段；在 Live 会话接入统一 billing settlement、余额扣减、订阅扣费、API Key 限额、平台配额、usage 真实成本和错误脱敏链路前，运行时必须保持不可用，服务端必须强制 `allow_live=false`，`/v1/live` 必须返回 503，管理端不能诱导保存启用状态。`session_id` 只能作为管理员排障字段，不得出现在用户侧 `/usage` DTO 中；新迁移必须支持旧库幂等升级，尤其非事务索引失败后需能清理重试。
...
- `deploy` 默认 `SUB2API_IMAGE`、Compose 默认镜像和本地 build 默认 tag 必须与 `backend/cmd/server/VERSION` 保持一致；v0.1.172 当前默认值为 `sub2api-custom:v0.1.172`。Release tag 和导出包可以带 `-custom`，但运行时 `VERSION` 与前端显示必须保持上游版本号，不得写成 `0.1.172-custom`。
