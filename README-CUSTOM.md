...
- 用户侧 `/subscriptions` 继续按分组聚合展示，但聚合卡片必须展示“有效可用额度”：同一张订阅卡需要同时满足每日、每周、每月、自定义窗口才算可用；某张卡被周/月/自定义窗口打满时，其每日剩余额也不能在用户侧显示为可用。重置时间按窗口分别计算：每日展示下一张能恢复每日有效额度的订阅卡时间，每周、每月、自定义同理；若目标窗口被其他窗口阻塞，展示解除阻塞后的有效恢复时间；若一张卡多个窗口同时阻塞，取该卡阻塞窗口中最晚的 reset，再在所有卡之间取最早可恢复点。`daily_usage_usd` 等原始字段保留真实聚合用量，`effective_*_usage_usd` 和 `effective_*_resets_at` 仅用于用户侧有效额度进度展示。不得暴露管理侧内部多条订阅卡细节、来源 ID 或兑换码快照。
