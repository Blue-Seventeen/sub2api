            ...
            <div class="space-y-1 text-sm">
              <div class="flex items-center gap-2">
                ...
                <div class="inline-flex items-center gap-1">
                  ...
                  <span class="font-medium text-gray-900 dark:text-white">{{ (hasImageOutputTokens(row) ? textOutputTokens(row) : (row.output_tokens ?? 0)).toLocaleString() }}</span>
                </div>
              </div>
        ...
        <div class="space-y-1.5">
          ...
          <div class="mb-2 border-b border-gray-700 pb-1.5">
            ...
            <div v-if="tooltipData && tooltipData.input_cost > 0" class="flex items-center justify-between gap-4">
              ...
              <span class="font-medium text-white">{{ formatCostAmount(tooltipData.input_cost, 6) }}</span>
            </div>
            ...
            <div v-if="tooltipData && tooltipData.output_cost > 0" class="flex items-center justify-between gap-4">
              ...
              <span class="font-medium text-white">{{ formatCostAmount(tooltipData.output_cost, 6) }}</span>
            </div>
            ...
            <template v-else-if="tooltipData && isImageUsage(tooltipData)">
              ...
              <div class="flex items-center justify-between gap-4">
                ...
                <span class="font-medium text-sky-300">{{ formatCostAmount(imageUnitPrice(tooltipData), 6) }}</span>
              </div>
              <div class="flex items-center justify-between gap-4">
            ...
            <div v-if="tooltipData && tooltipData.cache_creation_cost > 0" class="flex items-center justify-between gap-4">
              ...
              <span class="font-medium text-white">{{ formatCostAmount(tooltipData.cache_creation_cost, 6) }}</span>
            </div>
            <div v-if="tooltipData && tooltipData.cache_read_cost > 0" class="flex items-center justify-between gap-4">
              <span class="text-gray-400">{{ t('admin.usage.cacheReadCost') }}</span>
              <span class="font-medium text-white">{{ formatCostAmount(tooltipData.cache_read_cost, 6) }}</span>
            </div>
          </div>
          <!-- Rate and Summary -->
...
function accountBilled(row: { total_cost?: number | null; account_stats_cost?: number | null; account_rate_multiplier?: number | null } | null | undefined): number {
  const base = row?.account_stats_cost ?? row?.total_cost ?? 0
