  ...
  <BaseDialog :show="show" :title="operation === 'add' ? t('admin.users.deposit') : t('admin.users.withdraw')" width="narrow" @close="$emit('close')">
    <form v-if="user" id="balance-form" @submit.prevent="handleBalanceSubmit" class="space-y-5">
      <div class="flex items-center gap-3 rounded-xl bg-gray-50 p-4 dark:bg-dark-700">
        ...
        <div class="flex-1"><p class="font-medium text-gray-900">{{ user.email }}</p><p class="text-sm text-gray-500">{{ t('admin.users.displayBalanceLabel') }}: {{ formatMoneyBalance(getDisplayBalance(user)) }}</p><p class="text-xs text-gray-400">{{ t('admin.users.realBalanceLabel') }}: {{ formatMoneyBalance(getRealBalance(user)) }} · {{ t('admin.users.balanceOperationsUseReal') }}</p></div>
      </div>
      <div>
