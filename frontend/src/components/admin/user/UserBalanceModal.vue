  ...
  <BaseDialog :show="show" :title="operation === 'add' ? t('admin.users.deposit') : t('admin.users.withdraw')" width="narrow" @close="$emit('close')">
    <form v-if="user" id="balance-form" @submit.prevent="handleBalanceSubmit" class="space-y-5">
      <div class="flex items-center gap-3 rounded-xl bg-gray-50 p-4 dark:bg-dark-700">
        ...
        <div class="flex-1"><p class="font-medium text-gray-900 dark:text-gray-100">{{ user.email }}</p><p class="text-sm text-gray-500 dark:text-gray-400">{{ t('admin.users.displayBalanceLabel') }}: {{ formatMoneyBalance(getDisplayBalance(user)) }}</p><p class="text-xs text-gray-400 dark:text-gray-500">{{ t('admin.users.realBalanceLabel') }}: {{ formatMoneyBalance(getRealBalance(user)) }} · {{ t('admin.users.balanceOperationsUseReal') }}</p></div>
      </div>
      <div>
        <label class="input-label">{{ operation === 'add' ? t('admin.users.depositAmount') : t('admin.users.withdrawAmount') }}</label>
        <div class="relative flex gap-2">
          <div class="relative flex-1"><div class="absolute left-3 top-1/2 -translate-y-1/2 font-medium text-gray-500">{{ getDisplayCurrencySymbol() }}</div><input v-model.number="form.amount" type="number" step="any" min="0" required class="input pl-8" /></div>
      ...
      <div v-if="form.amount > 0" class="rounded-xl border border-blue-200 bg-blue-50 p-4 dark:border-blue-800 dark:bg-blue-950"><div class="flex items-center justify-between text-sm"><span class="text-gray-700 dark:text-gray-300">{{ t('admin.users.newBalance') }}:</span><span class="font-bold text-gray-900 dark:text-gray-100">{{ formatMoneyBalance(calculateNewBalance()) }}</span></div></div>
    </form>
    <template #footer>
...
const formatMoneyBalance = (value: number) => `${getDisplayCurrencySymbol()}${formatBalance(value)}`
