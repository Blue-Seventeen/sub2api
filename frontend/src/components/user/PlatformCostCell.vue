<template>
  <div v-if="usage" class="text-sm">
    <div class="flex items-center gap-1.5">
      ...
      <span class="font-medium text-gray-900 dark:text-white">{{ formatCostAmount(usage.today_actual_cost) }}</span>
    </div>
    <div class="mt-0.5 flex items-center gap-1.5">
      <span class="text-gray-500 dark:text-gray-400">{{ t('admin.users.total') }}:</span>
      <span class="font-medium text-gray-900 dark:text-white">{{ formatCostAmount(usage.total_actual_cost) }}</span>
    </div>
  </div>
