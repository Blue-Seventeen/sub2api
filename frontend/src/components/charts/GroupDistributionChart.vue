            ...
            <template v-for="group in displayGroupStats" :key="group.group_id">
              ...
              >
                ...
                <td v-if="showAccountCost" class="py-1.5 text-right text-orange-500 dark:text-orange-400">
                  {{ formatDisplayCost(group.account_cost) }}
                </td>
...
const displayGroupStats = computed(() => {
  ...
  return [...props.groupStats].sort((a, b) => toFiniteNumber(b.total_tokens) - toFiniteNumber(a.total_tokens))
})
...
const chartData = computed(() => {
  ...
  return {
    ...
    datasets: [
      {
        data: displayGroupStats.value.map((g) => props.metric === 'actual_cost' ? getGroupActualCost(g) : toFiniteNumber(g.total_tokens)),
...
const formatDisplayCost = (value: number | null | undefined): string => `${getDisplayCurrencySymbol()}${formatCost(value)}`
</script>
