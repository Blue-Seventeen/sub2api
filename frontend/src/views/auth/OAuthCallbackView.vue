        ...
        <div class="mt-6 space-y-4">
          ...
          <div>
            ...
            <div class="flex gap-2">
              ...
              <button class="btn btn-secondary" type="button" :disabled="!state" @click="copy(state)">
                {{ t('common.copy') }}
              </button>
            </div>
          </div>
          ...
          <div>
            ...
            <div class="flex gap-2">
              ...
              <button class="btn btn-secondary" type="button" :disabled="!fullUrl" @click="copy(fullUrl)">
                {{ t('common.copy') }}
              </button>
            </div>
          </div>
...
async function resumePendingEmailOAuth() {
  ...
  try {
    ...
    appStore.showError(err.response?.data?.message || err.message || t('auth.loginFailed'))
...
async function handleSubmitRegistration() {
  ...
  if (invitationRequired.value && !invitation) return
...
onMounted(async () => {
  ...
  const fragmentErrorDescription = params.get('error_description') || params.get('error_message') || ''
  ...
  if (route.path === '/auth/oauth/callback') {
    ...
    if (pendingEmailOAuthProvider) {
      pendingProvider.value = pendingEmailOAuthProvider
    }
