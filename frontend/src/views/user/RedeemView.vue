      ...
      <div class="card overflow-hidden">
        <div class="bg-gradient-to-br from-primary-500 to-primary-600 px-6 py-8 text-center">
          ...
          <p class="mt-2 text-4xl font-bold text-white">
            {{ formatCurrencyAmount(user?.balance ?? 0) }}
          </p>
                ...
                <div class="mt-2 text-sm text-emerald-700 dark:text-emerald-400">
                  ...
                  <div class="mt-3 space-y-1">
                    <p v-if="redeemResult.type === 'balance'" class="font-medium">
                      {{ t('redeem.added') }}: {{ formatCurrencyAmount(redeemResult.value) }}
                    </p>
                    ...
                    <p v-if="redeemResult.new_balance !== undefined">
                      {{ t('redeem.newBalance') }}:
                      <span class="font-semibold">{{ formatCurrencyAmount(redeemResult.new_balance) }}</span>
                    </p>
...
const isBalanceType = (type: string) => {
  return type === 'balance' || type === 'admin_balance' || type === 'promotion_balance'
}
...
const getHistoryItemTitle = (item: RedeemHistoryItem) => {
  ...
  } else if (item.type === 'promotion_balance') {
    return item.value >= 0 ? t('redeem.balanceAddedPromotion') : t('redeem.balanceDeductedPromotion')
...
const formatHistoryValue = (item: RedeemHistoryItem) => {
  if (isBalanceType(item.type)) {
    ...
    return `${sign}${formatCurrencyAmount(item.value)}`
