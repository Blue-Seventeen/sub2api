  ...
  <AppLayout>
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div v-show="activeTab === 'gateway'" class="space-y-6">
          ...
          <div class="card">
            ...
            <div class="space-y-5 p-6">
              ...
              >
                {{ t("admin.settings.openaiFastPolicy.flexForcedHint") }}
              </div>

              <div
                v-if="openaiFastPolicyLoading"
                class="flex items-center gap-2 text-gray-500"
              >
                <div
                  class="h-4 w-4 animate-spin rounded-full border-b-2 border-primary-600"
                  ...
                  <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                    <div>
                      ...
                      <Select
                        ...
                        @update:modelValue="
                          rule.service_tier = $event as OpenAIFastPolicyTier
                        "
        ...
        <div v-if="affiliateFeatureAvailable" class="card">
          <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
        ...
        <div
          v-if="affiliateFeatureAvailable && affiliateBatchModal.open"
          class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"
      ...
      <ConfirmDialog
        :show="affiliateFeatureAvailable && affiliateConfirmDialog.show"
...
const PLATFORM_QUOTA_PLATFORMS = ALLOWED_QUOTA_PLATFORMS;
...
const affiliateFeatureAvailable = false;
...
const authSourceDefaultsMeta = computed(() => [
  ...
  },
  {
    ...
    title: t("auth.googleProviderName"),
    description: localText(
...
async function loadSettings() {
  ...
  try {
    ...
    form.affiliate_enabled = false;
    form.affiliate_admin_recharge_enabled = false;
...
async function loadOpenAIFastPolicySettings() {
  ...
  try {
    ...
    openaiFastPolicyForm.rules = Array.isArray(settings.rules)
      ? settings.rules
          .filter((rule) => (rule.service_tier as string) !== "flex")
          .map((rule) => ({
            ...rule,
            user_ids: rule.user_ids ? [...rule.user_ids] : [],
            model_whitelist: rule.model_whitelist
...
async function saveOpenAIFastPolicySettings() {
  ...
  try {
    ...
    openaiFastPolicyForm.rules = updated.rules.map((rule) => ({
      ...rule,
      user_ids: rule.user_ids ? [...rule.user_ids] : [],
      model_whitelist: rule.model_whitelist ? [...rule.model_whitelist] : [],
    }));
    appStore.showSuccess(t("admin.settings.openaiFastPolicy.saved"));
  } catch (error: unknown) {
    appStore.showError(
      extractApiErrorMessage(
        error,
        t("admin.settings.openaiFastPolicy.saveFailed"),
      ),
    );
  } finally {
...
watch(
  ...
  (enabled, prev) => {
    if (affiliateFeatureAvailable && enabled && !prev) {
      loadAffiliateUsers();
    }
  },
);
