        ...
        >
          ...
          <template #cell-real_balance="{ row }">
            <div class="flex items-center gap-2">
              <div class="group relative">
                ...
                >
                  {{ formatCurrencyAmount(getAdminRealBalance(row)) }}
                </button>
                <!-- Instant tooltip -->
          ...
          <template #cell-balance="{ row }">
            <span class="font-medium text-gray-900 dark:text-white">
              {{ formatCurrencyAmount(getAdminDisplayBalance(row)) }}
            </span>
          </template>
  ...
  try {
    ...
    if (saved) {
      ...
      const normalized = new Set(
        parsed.filter(key => !REMOVED_COLUMNS.has(key) && !FORCED_VISIBLE_COLUMNS.has(key))
      )
      let migrated = normalized.size !== parsed.length
...
const loadInitialSortState = (): { sort_by: string; sort_order: 'asc' | 'desc' } => {
  ...
  const sortable = new Set(['email', 'id', 'username', 'role', 'balance', 'real_balance', 'concurrency', 'status', 'last_used_at', 'last_active_at', 'created_at'])
