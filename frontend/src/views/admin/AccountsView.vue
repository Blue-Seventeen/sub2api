<template>
  <AppLayout>
        ...
        >
          ...
          <template #cell-proxy="{ row }">
            <div class="flex flex-col gap-1">
              ...
              <span v-else-if="!row.proxy_auto_select_best" class="text-sm text-gray-400 dark:text-dark-500">-</span>
              ...
              <span
                v-if="!row.proxy && row.proxy_auto_select_best"
    ...
    <AccountAutoOpsModal v-if="showAutoOps" :show="showAutoOps" :highlight-run-id="autoOpsHighlightRunId" @close="showAutoOps = false" @saved="handleAutoOpsSaved" />
...
const AccountAutoOpsModal = defineAsyncComponent(() => import('@/components/admin/account/AccountAutoOpsModal.vue'))
...
const openBulkEditSelected = () => {
  const selectedRows = accounts.value.filter(account => selIds.value.includes(account.id))
  const { selectedPlatforms, selectedTypes } = collectSelectionMetadata(selectedRows)
  bulkEditTarget.value = {
    mode: 'selected',
    accountIds: [...selIds.value],
    selectedPlatforms: selectedPlatforms.length > 0 ? selectedPlatforms : [...selPlatforms.value],
    selectedTypes: selectedTypes.length > 0 ? selectedTypes : [...selTypes.value]
  }
