...
type UsageLog struct {
	...
	// 管理员真实消费口径（不受统一倍率放大影响）
	RealActualCost float64 `json:"real_actual_cost,omitempty"`
	// 写入日志时的用户统一倍率快照
...
func (*UsageLog) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case usagelog.FieldImageSizeBreakdown:
			values[i] = new([]byte)
		case usagelog.FieldUpstreamModelMismatch, usagelog.FieldLongContextBillingApplied, usagelog.FieldStream, usagelog.FieldUsageEstimated, usagelog.FieldCacheTTLOverridden:
...
func (_m *UsageLog) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case usagelog.FieldRealActualCost:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field real_actual_cost", values[i])
			} else if value.Valid {
				_m.RealActualCost = value.Float64
			}
...
func (_m *UsageLog) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("real_actual_cost=")
	builder.WriteString(fmt.Sprintf("%v", _m.RealActualCost))
	builder.WriteString(", ")
