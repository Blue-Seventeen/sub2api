...
// ErrUserPlatformQuotaCacheNotReady means the Redis quota hash was missing or
// used an older schema, so callers that rely on Redis as the persistence source
// must fall back to the database for this increment.
var ErrUserPlatformQuotaCacheNotReady = errors.New("user platform quota cache not ready")
...
type CostBreakdown struct {
	...
	RealActualCost            float64 // Admin-facing real billed cost.
...
func (s *BillingService) CalculateCostUnified(input CostInput) (*CostBreakdown, error) {
	...
	case BillingModeCharacter:
		breakdown, err = s.calculateCharacterCost(resolved, input)
...
func (s *BillingService) calculateCharacterCost(resolved *ResolvedPricing, input CostInput) (*CostBreakdown, error) {
	...
	totalCost := resolved.DefaultPerRequestPrice * (float64(count) / 1000.0)
	actualCost := totalCost * input.RateMultiplier
	return &CostBreakdown{
		TotalCost:  totalCost,
		ActualCost: actualCost,
