...
func TestAssignOrExtendSubscriptionUsesLockedCurrentRow(t *testing.T) {
	...
	svc := NewSubscriptionService(&subscriptionGroupRepoStub{group: &Group{ID: 13, Status: StatusActive, SubscriptionType: SubscriptionTypeSubscription}}, repo, nil, nil, nil)
...
func TestAssignOrExtendSubscriptionSerializedRenewalsAccumulateDays(t *testing.T) {
	...
	svc := NewSubscriptionService(&subscriptionGroupRepoStub{group: &Group{ID: 23, Status: StatusActive, SubscriptionType: SubscriptionTypeSubscription}}, repo, nil, nil, nil)
	svc.now = func() time.Time { return now }
...
func TestAssignSubscriptionDoesNotReactivateRowSuspendedAfterStaleRead(t *testing.T) {
	...
	svc := NewSubscriptionService(&subscriptionGroupRepoStub{group: &Group{ID: 33, Status: StatusActive, SubscriptionType: SubscriptionTypeSubscription}}, repo, nil, nil, nil)
	svc.now = func() time.Time { return now }
