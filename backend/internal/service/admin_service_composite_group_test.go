...
func TestAdminService_CompositeModelsListCandidatesIncludeConcreteAccountMappings(t *testing.T) {
	...
	svc := &adminServiceImpl{accountRepo: accountRepo, groupRepo: groupRepo, compositeRouteRepo: routeRepo}
	...
	require.Contains(t, candidates, "all/kimi")
	require.NotContains(t, candidates, "all/router/")
	require.Contains(t, candidates, "gpt-5.5")
	require.Contains(t, candidates, "gemini-2.5-flash")
	require.Contains(t, candidates, "kimi-k2.6")
}
