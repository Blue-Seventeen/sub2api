...
func TestShouldInferIngressFunctionCallOutputPreviousResponseID(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:             "skip_when_matching_tool_call_context_already_present",
			storeDisabled:    true,
			turn:             2,
			signals:          ToolContinuationSignals{HasFunctionCallOutput: true, HasToolCallContext: true, HasToolCallContextForAllCallIDs: true},
			expectedPrevious: "resp_2",
			want:             false,
		},
		{
			name:             "infer_when_tool_call_context_does_not_match_output_call_id",
			storeDisabled:    true,
			turn:             2,
			signals:          ToolContinuationSignals{HasFunctionCallOutput: true, HasToolCallContext: true, HasToolCallContextForAllCallIDs: false},
			expectedPrevious: "resp_2",
			want:             true,
		},
		{
