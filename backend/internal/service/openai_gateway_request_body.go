...
func (s *OpenAIGatewayService) applyOpenAIFastPolicyToBody(ctx context.Context, account *Account, model string, body []byte) ([]byte, *string, error) {
	...
	rawTierValue := rawTier.String()
	normTier := normalizedOpenAIServiceTierValue(rawTierValue)
	if normTier == "" {
		return body, nil, nil
	}
	...
	case OpenAIFastPolicyActionForcePriority:
		...
		if err != nil {
			return body, nil, fmt.Errorf("force service_tier priority on body: %w", err)
		}
		tier := OpenAIFastTierPriority
		return updated, &tier, nil
	default:
		// pass：把别名（如 "fast"）写回为规范值（"priority"）。
		if normTier == rawTierValue {
