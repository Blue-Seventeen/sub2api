...
func TestAdminService_UpdateUserBalance_AdminRechargeAffiliateDisabledInFork(t *testing.T) {
	tests := []struct {
		name      string
		enabled   bool
		operation string
...
func TestAdminService_UpdateUserBalance_AdminRechargeAffiliateSettingDoesNotAffectRecharge(t *testing.T) {
	baseRepo := &userRepoStub{user: &User{ID: 7, Balance: 10}}
	repo := &balanceUserRepoStub{userRepoStub: baseRepo}
	redeemRepo := &balanceRedeemRepoStub{redeemRepoStub: &redeemRepoStub{}}
	svc := &adminServiceImpl{
