...
type RecordUsageInput struct {
	...
	CompatibleInputTokens int    // compatible usage fallback 的请求 token 估算快照，避免 worker 持有 ParsedRequest/RequestBodyRef
...
func postUsageBilling(ctx context.Context, p *postUsageBillingParams, deps *billingDeps) {
	...
	if p.IsSubscriptionBill {
		...
		if cost.RealActualCost > 0 {
			if err := deps.userRepo.DeductBalance(billingCtx, p.User.ID, cost.RealActualCost); err != nil {
			...
			deps.billingCacheService.QueueDeductBalance(p.User.ID, cost.RealActualCost)
		}
	}
	...
	if !p.IsSubscriptionBill && p.Platform != "" && cost.ActualCost > 0 && p.User != nil && deps.userPlatformQuotaRepo != nil {
		if deps.billingCacheService.HasUserPlatformQuotaLimit(billingCtx, p.User.ID, p.Platform) {
			cacheUpdated := deps.billingCacheService.IncrementUserPlatformQuotaUsage(p.User.ID, p.Platform, cost.ActualCost)
			if deps.cfg == nil || !deps.cfg.Database.UserPlatformQuotaFlusherEnabled || !cacheUpdated {
...
func finalizePostUsageBilling(ctx context.Context, p *postUsageBillingParams, deps *billingDeps, result *UsageBillingApplyResult) {
	...
	if !p.IsSubscriptionBill && p.Platform != "" && p.Cost.ActualCost > 0 && p.User != nil && deps.userPlatformQuotaRepo != nil {
		if deps.billingCacheService.HasUserPlatformQuotaLimit(ctx, p.User.ID, p.Platform) {
			cacheUpdated := deps.billingCacheService.IncrementUserPlatformQuotaUsage(p.User.ID, p.Platform, p.Cost.ActualCost)
			if deps.cfg == nil || !deps.cfg.Database.UserPlatformQuotaFlusherEnabled || !cacheUpdated {
...
func notifyBalanceLow(p *postUsageBillingParams, deps *billingDeps, result *UsageBillingApplyResult) {
	...
	if p.IsSubscriptionBill || p.Cost.RealActualCost <= 0 || p.User == nil || deps.balanceNotifyService == nil {
		slog.Debug("notifyBalanceLow: skipped",
			"is_subscription", p.IsSubscriptionBill,
			"real_actual_cost", p.Cost.RealActualCost,
			"user_nil", p.User == nil,
	...
	slog.Debug("notifyBalanceLow: calling CheckBalanceAfterDeduction",
		...
		"cost", p.Cost.RealActualCost,
		"notify_enabled", p.User.BalanceNotifyEnabled,
		"threshold", p.User.BalanceNotifyThreshold,
		"result_has_new_balance", result != nil && result.NewBalance != nil,
	)
	deps.balanceNotifyService.CheckBalanceAfterDeduction(context.Background(), p.User, oldBalance, p.Cost.RealActualCost)
}
...
func resolveOldBalance(p *postUsageBillingParams, result *UsageBillingApplyResult) float64 {
	if result != nil && result.NewBalance != nil {
		return *result.NewBalance + p.Cost.RealActualCost
	}
...
func (s *GatewayService) RecordUsage(ctx context.Context, input *RecordUsageInput) error {
	...
	return s.recordUsageCore(ctx, &recordUsageCoreInput{
		...
		CompatibleInputTokens: input.CompatibleInputTokens,
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput, opts *recordUsageOpts) error {
	...
	applyCompatibleUsageFallback(result, account, apiKey.Group, input.CompatibleInputTokens)
	...
	usageLog := s.buildRecordUsageLog(ctx, input, result, apiKey, user, account, subscription,
		requestedModel, multiplier, imageMultiplier, unifiedMultiplier, accountRateMultiplier, billingType, cacheTTLOverridden, cost, opts)
