...
func (s *GatewayService) BindStickySession(ctx context.Context, groupID *int64, sessionHash string, accountID int64) error {
	...
	if existingID, err := s.cache.GetSessionAccountID(ctx, gid, sessionHash); err == nil && existingID > 0 {
		if existingID == accountID {
			return s.cache.RefreshSessionTTL(ctx, gid, sessionHash, stickySessionTTL)
		}
		return nil
	}
...
func (s *GatewayService) debugLogGatewaySnapshot(tag string, headers http.Header, body []byte, extra map[string]string) {
	...
	} else if !s.debugGatewayBodyUnsafeFull.Load() {
		if json.Valid(body) {
			fmt.Fprintf(&buf, "  [redacted json body] bytes=%d\n", len(body))
		} else {
			fmt.Fprintf(&buf, "  [redacted non-json body] bytes=%d\n", len(body))
		}
	} else {
