...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	parseClientPayload := func(turn int, raw []byte) (openAIWSClientPayload, error) {
		...
		policyApplied, _, blocked, policyErr := s.applyOpenAIFastPolicyToWSResponseCreate(ctx, account, upstreamModel, normalized)
		if policyErr != nil {
		...
		if account != nil && account.Type == AccountTypeAPIKey && gjson.GetBytes(normalized, "service_tier").Exists() {
			stripped, stripErr := sjson.DeleteBytes(normalized, "service_tier")
			if stripErr != nil {
				return openAIWSClientPayload{}, NewOpenAIWSClientCloseError(coderws.StatusPolicyViolation, "invalid websocket request payload", stripErr)
			}
			normalized = stripped
