...
func TestForwardAsAnthropic_NormalizesRoutingAndEffortForGpt54XHigh(t *testing.T) {
	...
	require.Equal(t, CompatibilityRouteAnthropicResponsesBridge, GetCompatibilityRoute(c))
	require.Equal(t, UpstreamTransportSSE, GetCompatibilityUpstreamTransport(c))
	require.Equal(t, []string{"bridge"}, GetCompatibilityFallbackStages(c))
	rawUpstreamBody, ok := c.Get(OpsUpstreamRequestBodyKey)
	require.True(t, ok)
	bodyBytes, ok := rawUpstreamBody.([]byte)
	require.True(t, ok)
	require.Equal(t, "gpt-5.4", gjson.GetBytes(bodyBytes, "model").String())
