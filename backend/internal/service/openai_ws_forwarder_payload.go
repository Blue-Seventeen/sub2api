...
func shouldInferIngressFunctionCallOutputPreviousResponseID(
	...
	// Only skip inference when every output call_id is already covered by a
	// matching tool-call context; partial context still needs the prior anchor.
	if signals.HasToolCallContext && signals.HasToolCallContextForAllCallIDs {
		return false
	}
