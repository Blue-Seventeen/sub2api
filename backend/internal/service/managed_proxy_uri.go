...
// parseProxyURIListSubscription 解析「base64 / 明文 URI 列表」型订阅（非 Clash YAML）。
//
// 许多机场（V2Board/Xboard 等）在 UA/flag 不满足时返回的是 base64 编码的 URI 列表
// （vmess:// / vless:// / ss:// / trojan:// / hysteria2:// / tuic://）。此函数尽力将其转成
// Clash proxy map，供上层复用现有解析/校验/持久化流程。
//
// 采用 best-effort 策略：无法识别或字段缺失的单条 URI 直接跳过，不因个别协议失败而整单失败；
// 返回空切片表示「不是可识别的 URI 列表订阅」。
...
// decodeMaybeBase64Subscription 返回订阅正文：若已是 URI 列表则原样返回，否则尝试 base64 解码。
...
func parseVMessURI(uri string) (map[string]any, bool) {
	...
	case "ws":
		proxy["network"] = "ws"
		if opts := wsOpts(path, host); len(opts) > 0 {
			proxy["ws-opts"] = opts
		}
	case "grpc":
		proxy["network"] = "grpc"
...
func applyTransportOpts(proxy map[string]any, q url.Values) {
	...
	case "ws":
		proxy["network"] = "ws"
		if opts := wsOpts(q.Get("path"), q.Get("host")); len(opts) > 0 {
			proxy["ws-opts"] = opts
		}
	case "grpc":
		proxy["network"] = "grpc"
