...
func TestCompositeRouteResolverExplicitRouteCanTargetCompatiblePlatform(t *testing.T) {
	resolver := NewCompositeRouteResolver(compositeRouteRepoStub{
		routes: []CompositeModelRoute{
			{
				ID:             5,
				...
				MatchType:      CompositeRouteMatchExact,
				TargetPlatform: PlatformMoonshot,
	...
	decision, err := resolver.Resolve(context.Background(), 7, "all/kimi", CompositeRouteEndpointMessages)

	require.NoError(t, err)
	require.True(t, decision.Matched)
	require.Equal(t, CompositeRouteSourceExplicit, decision.Source)
	require.Equal(t, PlatformMoonshot, decision.TargetPlatform)
	require.Equal(t, "kimi-k2.6", decision.UpstreamModel)
}

func TestCompositeRouteResolverListExactPublicModels(t *testing.T) {
	resolver := NewCompositeRouteResolver(compositeRouteRepoStub{
		routes: []CompositeModelRoute{
			{
				GroupID:        7,
			...
			},
			{
				...
				MatchType:      CompositeRouteMatchPrefix,
				TargetPlatform: PlatformOpenRouter,
	...
	models, err := resolver.ListExactPublicModels(context.Background(), 7)

	require.NoError(t, err)
	require.Equal(t, []string{"all/kimi"}, models)
}
