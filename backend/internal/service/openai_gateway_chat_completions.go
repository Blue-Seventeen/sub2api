...
func (s *OpenAIGatewayService) handleChatStreamingResponse(
	...
	processDataLine := func(payload string) bool {
		...
		if !clientDisconnected {
			if len(chunks) > 0 {
				if c == nil || c.Writer == nil || !c.Writer.Written() {
					if !chatChunksHaveSubstantiveOutput(chunks) {
						pendingChunks = append(pendingChunks, chunks...)
						return isTerminalEvent
					}
					if len(pendingChunks) > 0 {
						chunks = append(append([]apicompat.ChatCompletionsChunk{}, pendingChunks...), chunks...)
						pendingChunks = nil
					}
				}
				writeStreamHeaders()
	...
	finalizeStream := func() (*OpenAIForwardResult, error) {
		...
		if finalChunks := apicompat.FinalizeResponsesChatStream(state); len(finalChunks) > 0 && !clientDisconnected {
			if len(pendingChunks) > 0 {
				finalChunks = append(append([]apicompat.ChatCompletionsChunk{}, pendingChunks...), finalChunks...)
				pendingChunks = nil
			}
			writeStreamHeaders()
	...
	currentSSEEventType := ""
	prepareSSEDataPayload := func(line string) (string, bool) {
		...
		return openAICompatPayloadWithEventType(payload, currentSSEEventType), true
	}
	...
	if streamInterval <= 0 && keepaliveInterval <= 0 {
		for scanner.Scan() {
			line := scanner.Text()
			payload, ok := prepareSSEDataPayload(line)
			if !ok {
				continue
			}
...
func chatChunksHaveSubstantiveOutput(chunks []apicompat.ChatCompletionsChunk) bool {
	for _, chunk := range chunks {
		if chunk.Usage != nil {
			return true
		}
