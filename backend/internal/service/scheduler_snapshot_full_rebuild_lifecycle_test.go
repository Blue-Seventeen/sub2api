...
func TestSchedulerFullRebuildActiveTombstoneDoesNotBlockFollowingGroupEvent(t *testing.T) {
	...
	require.Equal(t, expectedSchedulerQueryCount(schedulerCanonicalBuckets(0))+2*expectedSchedulerQueryCount(canonical), accounts.callCount())
}
...
func TestSchedulerFullRebuildFreshActivePreparesEveryTokenBeforeFirstDB(t *testing.T) {
	...
	current := schedulerBucketsForGroup(groupID)
	groupZero := schedulerCanonicalBuckets(0)
	accounts.beforeFirst = func() {
		capturesAtFirstDB = cache.captureAttemptCount()
		held, reopenCount := cache.leaseHeldAndTokenCount()
		require.False(t, held)
		require.Equal(t, len(current), reopenCount)
		require.Equal(t, len(groupZero)+1, capturesAtFirstDB, "C(0) and the historical bucket must be captured before DB")
	}
	svc := newFullRebuildLifecycleService(cache, nil, accounts, groups, config.RunModeStandard)
	...
	require.Equal(t, expectedSchedulerQueryCount(groupZero)+expectedSchedulerQueryCount(current)+1, accounts.callCount())
	require.Equal(t, expectedSchedulerQueryCount(current)+1, accounts.groupCallCount(groupID))
...
func TestSchedulerFullRebuildPreservesGroupZeroActiveHistoricalAndInvalidRegistryBuckets(t *testing.T) {
	...
	require.NoError(t, svc.rebuildFullSnapshot(context.Background(), "test"))
	expectedBuckets := append(append([]SchedulerBucket{}, schedulerCanonicalBuckets(0)...), schedulerBucketsForGroup(groupID)...)
	expectedBuckets = append(expectedBuckets, groupZeroHistorical, activeHistorical, invalidHistorical)
	require.Equal(t, len(expectedBuckets), cache.captureAttemptCount())
	require.Equal(t, expectedSchedulerQueryCount(expectedBuckets), accounts.callCount())
	groups.mu.Lock()
	require.Equal(t, 1, groups.listCalls)
	groups.mu.Unlock()
...
func TestSchedulerFullRebuildActiveTombstoneFreshInactiveOrMissingFiltersAllGroupTasks(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, expectedSchedulerQueryCount(schedulerCanonicalBuckets(0)), accounts.groupCallCount(0))
			require.Empty(t, cache.tokens())
...
func TestSchedulerFullRebuildPartialLifecycleFailureReturnsBeforeDBAndRetries(t *testing.T) {
	...
	require.Zero(t, accounts.callCount())
	require.Zero(t, cache.totalSetAttempts())
	perInactiveGroupRetireCount := len(schedulerBucketsForGroup(1)) + 1
	require.Equal(t, perInactiveGroupRetireCount, len(cache.retiredBuckets()))
	...
	require.Equal(t, 3*perInactiveGroupRetireCount, len(cache.retiredBuckets()))
	require.Equal(t, expectedSchedulerQueryCount(schedulerCanonicalBuckets(0)), accounts.callCount())
	require.Empty(t, cache.tokens())
}
...
func TestSchedulerFullRebuildActiveTombstoneLazyRecoveryDiscardsPartialCaptureTasks(t *testing.T) {
	...
	accounts.beforeFirst = func() {
		...
		require.GreaterOrEqual(t, capturesAtFirstDB, len(groupZero))
	}
	svc := newFullRebuildLifecycleService(cache, nil, accounts, groups, config.RunModeStandard)

	require.NoError(t, svc.rebuildFullSnapshot(context.Background(), "test"))
	require.Equal(t, capturesAtFirstDB, cache.captureAttemptCount())
	require.Equal(t, expectedSchedulerQueryCount(groupZero)+expectedSchedulerQueryCount(canonical)+1, accounts.callCount())
...
func TestSchedulerFullRebuildSimpleModePreservesRegistryWithoutLifecycleAuthority(t *testing.T) {
	...
	require.Zero(t, fallbackCalls)
	require.Empty(t, freshCalls)
	expectedBuckets := append(append([]SchedulerBucket{}, schedulerCanonicalBuckets(0)...), registered...)
	require.Equal(t, len(expectedBuckets), cache.captureAttemptCount())
	require.Equal(t, expectedSchedulerQueryCount(expectedBuckets), accounts.callCount())
	require.Empty(t, cache.retiredBuckets())
	require.Empty(t, cache.tokens())
...
func TestSchedulerFullRebuildFreshReopenLockBusyRetriesWithoutBlockingOrdinaryTasks(t *testing.T) {
	...
	svc := newFullRebuildLifecycleService(cache, outbox, accounts, groups, config.RunModeStandard)
	perFullRebuildQueryCount := expectedSchedulerQueryCount(schedulerCanonicalBuckets(0)) + expectedSchedulerQueryCount(canonical)
	...
	require.Equal(t, perFullRebuildQueryCount, accounts.callCount())

	svc.pollOutbox()
	require.Equal(t, int64(1), cache.currentWatermark())
	require.Equal(t, 2*perFullRebuildQueryCount, accounts.callCount())
