...
func (f *fakeIncrCache) DeleteUserPlatformQuotaCache(ctx context.Context, userID int64, platform string) error {
	f.deleteCalls++
	return nil
}
...
func TestIncrementUserPlatformQuotaUsage_SyncCallsCache(t *testing.T) {
	...
	if ok := s.IncrementUserPlatformQuotaUsage(101, "anthropic", 0.25); !ok {
		t.Fatal("expected first cache increment to succeed")
	}
	if ok := s.IncrementUserPlatformQuotaUsage(101, "openai", 0.50); !ok {
		t.Fatal("expected second cache increment to succeed")
	}
...
func TestIncrementUserPlatformQuotaUsage_CacheNotReadyReturnsFalseWithoutInvalidating(t *testing.T) {
	...
	if ok := s.IncrementUserPlatformQuotaUsage(101, "openai", 0.50); ok {
		t.Fatal("expected cache increment to report failure")
	}
