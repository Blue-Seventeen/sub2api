...
func TestCalculateOpenAIRecordUsageCostWebSearchPerCall(t *testing.T) {
	...
	// 分组未配置单价：默认 0.01。按次搜索使用包含高峰因子的最终倍率。
	...
	cost, err := svc.calculateOpenAIRecordUsageCost(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, 3.0, 1.0, 1.0, UsageTokens{}, "", false)
	...
	cost, err = svc.calculateOpenAIRecordUsageCost(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, 1.0, 1.0, 1.0, UsageTokens{}, "", false)
	require.NoError(t, err)
	...
	_, err = svc.calculateOpenAIRecordUsageCost(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, 1.0, 1.0, 1.0, UsageTokens{InputTokens: 10}, "", false)
	require.Error(t, err)
}
