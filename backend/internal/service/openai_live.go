...
func (s *OpenAIGatewayService) createUpstreamLiveCall(
	...
	resp, err := s.httpUpstream.Do(upstreamReq, resolveAccountProxyURL(ctx, account, nil), account.ID, account.Concurrency)
	if err != nil {
...
func logLiveUpstreamFailure(
	...
	logger.FromContext(ctx).Warn(
		...
		zap.String("upstream_error_type", truncateOpenAIWSLogValue(logredact.RedactText(errorType), 120)),
		zap.String("upstream_error_code", truncateOpenAIWSLogValue(logredact.RedactText(errorCode), 120)),
		zap.String("upstream_error_message", truncateOpenAIWSLogValue(logredact.RedactText(errorMessage, "key", "subscription_url"), 300)),
...
func (s *OpenAIGatewayService) dialLiveSideband(ctx context.Context, record *LiveCallRecord) (liveFrameConn, error) {
	...
	conn, status, _, err := s.getOpenAIWSPassthroughDialer().Dial(ctx, target, headers, resolveAccountProxyURL(ctx, account, nil))
	if err != nil {
