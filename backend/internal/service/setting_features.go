...
const (
	openAIFastPolicySettingsCacheTTL  = 60 * time.Second
	openAIFastPolicySettingsErrorTTL  = 5 * time.Second
	openAIFastPolicySettingsDBTimeout = 5 * time.Second
)
...
func (s *SettingService) GetOpenAIFastPolicySettings(ctx context.Context) (*OpenAIFastPolicySettings, error) {
	...
	if err != nil {
		...
		if stale != nil && stale.settings != nil {
			cacheOpenAIFastPolicySettings(stale.settings, openAIFastPolicySettingsErrorTTL)
			return cloneOpenAIFastPolicySettings(stale.settings), nil
		}
		settings := DefaultOpenAIFastPolicySettings()
	...
	return cloneOpenAIFastPolicySettings(normalized), nil
}
