...
func (s *adminServiceImpl) GetGroupModelsListCandidates(ctx context.Context, id int64, platform string) ([]string, error) {
	...
	if platform == PlatformComposite && id > 0 && s.compositeRouteRepo != nil {
		routeModels, err := NewCompositeRouteResolver(s.compositeRouteRepo).ListExactPublicModels(ctx, id)
		if err != nil {
			return nil, err
		}
		candidates = appendUniqueModelIDs(candidates, routeModels...)
	}
	...
	for _, acc := range accounts {
		...
		candidates = appendUniqueModelIDsWithSeen(candidates, seen, accountModelMappingKeys(acc)...)
	}
	return candidates, nil
}
...
func compositeDefaultModelsListCandidateIDs() []string {
	...
	ids := make([]string, 0)
	for _, platform := range ConcreteRequestPlatforms() {
...
func accountModelMappingKeys(acc Account) []string {
...
func appendUniqueModelIDs(models []string, candidates ...string) []string {
	seen := make(map[string]struct{}, len(models)+len(candidates))
	for _, model := range models {
	...
	return appendUniqueModelIDsWithSeen(models, seen, candidates...)
}

func appendUniqueModelIDsWithSeen(models []string, seen map[string]struct{}, candidates ...string) []string {
	for _, model := range candidates {
		model = strings.TrimSpace(model)
		if model == "" {
			continue
...
func (s *adminServiceImpl) CreateGroup(ctx context.Context, input *CreateGroupInput) (*Group, error) {
	...
	if len(input.PeakRateWindows) > 0 && !input.PeakRateEnabledSet {
		peakRateEnabled = true
	}
	peakRateWindows := input.PeakRateWindows
	if len(peakRateWindows) == 0 {
		peakRateWindows = singlePeakRateWindowFromLegacy(input.PeakStart, input.PeakEnd, peakRateMultiplier)
	}
	normalizedPeakWindows, err := NormalizePeakRateWindows(peakRateEnabled, peakRateWindows)
	if err != nil {
		return nil, infraerrors.BadRequest("INVALID_PEAK_RATE_WINDOWS", err.Error())
	}
	peakStart, peakEnd, peakRateMultiplier := PeakRateLegacyFields(normalizedPeakWindows)
	peakRateEnabled = peakRateEnabled && len(normalizedPeakWindows) > 0
	...
	group := &Group{
		...
		CustomLimitUSD:                  normalizeLimit(input.CustomLimitUSD),
		...
		PeakRateWindows:                 normalizedPeakWindows,
...
func (s *adminServiceImpl) UpdateGroup(ctx context.Context, id int64, input *UpdateGroupInput) (*Group, error) {
	...
	peakRateWindows := PeakRateWindowsForRead(group.PeakRateWindows, group.PeakStart, group.PeakEnd, group.PeakRateMultiplier)
	if input.PeakRateWindows != nil {
		peakRateWindows = *input.PeakRateWindows
	} else if input.PeakStart != nil || input.PeakEnd != nil || input.PeakRateMultiplier != nil {
		if len(peakRateWindows) == 0 {
			peakRateWindows = singlePeakRateWindowFromLegacy(group.PeakStart, group.PeakEnd, group.PeakRateMultiplier)
		}
		if len(peakRateWindows) > 0 {
			first := peakRateWindows[0]
			if input.PeakStart != nil {
				first.Start = *input.PeakStart
			}
			if input.PeakEnd != nil {
				first.End = *input.PeakEnd
			}
			if input.PeakRateMultiplier != nil {
				first.Multiplier = *input.PeakRateMultiplier
			}
			peakRateWindows[0] = first
		}
	}
	normalizedPeakWindows, err := NormalizePeakRateWindows(group.PeakRateEnabled, peakRateWindows)
	if err != nil {
		return nil, infraerrors.BadRequest("INVALID_PEAK_RATE_WINDOWS", err.Error())
	}
	group.PeakRateWindows = normalizedPeakWindows
	group.PeakStart, group.PeakEnd, group.PeakRateMultiplier = PeakRateLegacyFields(normalizedPeakWindows)
	group.PeakRateEnabled = group.PeakRateEnabled && len(normalizedPeakWindows) > 0
