...
func expectedGroupLifecycleBuckets(groupID int64) []SchedulerBucket {
	return schedulerBucketsForGroup(groupID)
}

func expectedSchedulerQueryCount(buckets []SchedulerBucket) int {
	seen := make(map[schedulerAccountQueryKey]struct{}, len(buckets))
	count := 0
...
func expectedGroupLifecycleQueryCount(groupID int64) int {
	return expectedSchedulerQueryCount(expectedGroupLifecycleBuckets(groupID))
}
...
func TestSchedulerGroupLifecycleActiveReopensAndRebuildsAllCurrentBuckets(t *testing.T) {
	...
	require.Equal(t, expectedGroupLifecycleQueryCount(groupID), accounts.callCount())
...
func TestSchedulerGroupLifecycleEpochPreventsABA(t *testing.T) {
	...
	current := expectedGroupLifecycleBuckets(groupID)
	require.Len(t, firstActiveTokens, len(current))
	...
	require.Len(t, allTokens, 2*len(current))
	require.Greater(t, allTokens[len(current)].Epoch, firstActiveTokens[0].Epoch)
