	...
	for _, choice := range chunk.Choices {
		...
		for _, toolCall := range choice.Delta.ToolCalls {
			...
			if toolCall.Function.Arguments != "" {
				prevArgs := stored.Function.Arguments
				stored.Function.Arguments = sanitizeToolCallArgumentsJSON(prevArgs + toolCall.Function.Arguments)
				deltaArgs := stored.Function.Arguments
				if prevArgs != "" && strings.HasPrefix(stored.Function.Arguments, prevArgs) {
					deltaArgs = stored.Function.Arguments[len(prevArgs):]
				}
				if deltaArgs == "" {
					continue
				}
