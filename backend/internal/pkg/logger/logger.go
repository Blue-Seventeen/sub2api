...
func initLocked(options InitOptions) error {
	...
	zl, al, cleanup, err := buildLogger(normalized)
	if err != nil {
		return err
	}

	prev := global.Load()
	prevCleanup := loggerCleanup
	global.Store(zl)
...
func buildLogger(options InitOptions) (*zap.Logger, zap.AtomicLevel, func(), error) {
	...
	if options.Output.ToStdout {
		...
		cores = append(cores, zapcore.NewCore(enc, zapcore.Lock(nopSyncWriter{Writer: os.Stdout}), infoPriority))
		cores = append(cores, zapcore.NewCore(enc, zapcore.Lock(nopSyncWriter{Writer: os.Stderr}), errPriority))
	}

	if options.Output.ToFile {
		fileCore, filePath, fileCleanup, fileErr := buildFileCore(enc, atomic, options)
		if fileErr != nil {
			...
			if fileCleanup != nil {
				cleanupFns = append(cleanupFns, fileCleanup)
			}
		}
	}

	if len(cores) == 0 {
		cores = append(cores, zapcore.NewCore(enc, zapcore.Lock(nopSyncWriter{Writer: os.Stdout}), atomic))
	}
	...
	return logger, atomic, cleanup, nil
}

func buildFileCore(enc zapcore.Encoder, atomic zap.AtomicLevel, options InitOptions) (zapcore.Core, string, func(), error) {
	...
	return zapcore.NewCore(enc, zapcore.AddSync(lj), atomic), filePath, func() {
		_ = lj.Close()
	}, nil
}
