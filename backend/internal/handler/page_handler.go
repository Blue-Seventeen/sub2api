...
func (h *PageHandler) enabled(c *gin.Context) bool {
	return h != nil && h.settingService != nil && h.settingService.IsMarkdownPagesEnabled(c.Request.Context())
}

// GetPageContent serves raw markdown content for a configured page slug.
func (h *PageHandler) GetPageContent(c *gin.Context) {
	...
	slug := c.Param("slug")
	if !validPageSlugPattern.MatchString(slug) || len(slug) > 64 {
