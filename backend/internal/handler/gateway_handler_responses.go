...
func (h *GatewayHandler) Responses(c *gin.Context) {
	...
	if publicModel, allowed := groupAllowsClientRequestedModel(c, apiKey.Group, reqModel); !allowed {
		h.responsesErrorResponse(c, http.StatusForbidden, "permission_error", groupModelsListDisallowedMessage(publicModel))
		return
	}
	// 解析渠道级模型映射
	...
	// 2. Re-check billing
	if err := h.billingCacheService.CheckBillingEligibilityFreshSubscription(requestCtx, apiKey.User, apiKey, apiKey.Group, service.QuotaPlatform(requestCtx, apiKey)); err != nil {
		...
		if err != nil {
			...
			default:
				if fs.LastFailoverErr != nil {
					recordGatewayProxyFailureStat(c.Request.Context(), h.gatewayService, h.proxyStatsWorkerPool, lastFailedAccount, apiKey, lastFailedDurationMs, "handler.gateway.responses")
		...
		if err != nil {
			...
			if errors.As(err, &failoverErr) {
				...
				if c.Writer.Size() != writerSizeBeforeForward {
					recordGatewayProxyFailureStat(c.Request.Context(), h.gatewayService, h.proxyStatsWorkerPool, account, apiKey, forwardDurationMs, "handler.gateway.responses")
