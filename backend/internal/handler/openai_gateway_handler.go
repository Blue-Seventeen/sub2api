...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	if publicModel, allowed := groupAllowsClientRequestedModel(c, apiKey.Group, reqModel); !allowed {
	...
	if err := h.billingCacheService.CheckBillingEligibilityFreshSubscription(c.Request.Context(), apiKey.User, apiKey, apiKey.Group, service.QuotaPlatform(c.Request.Context(), apiKey)); err != nil {
	...
	for {
		...
		result, upstreamEndpoint, err = func() (*service.OpenAIForwardResult, string, error) {
			...
			if h.newAPIStyleService != nil && h.newAPIStyleService.SupportsForGroup(account, apiKey.Group, service.NewAPIStyleRouteResponses) {
				return h.newAPIStyleService.ForwardOpenAI(
					c.Request.Context(),
					c,
					account,
...
func (h *OpenAIGatewayHandler) Messages(c *gin.Context) {
	...
	if publicModel, allowed := groupAllowsClientRequestedModel(c, apiKey.Group, reqModel); !allowed {
		h.anthropicErrorResponse(c, http.StatusForbidden, "permission_error", groupModelsListDisallowedMessage(publicModel))
		return
	}
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	setOpsRequestContext(c, clientRequestedModel(c, reqModel), true)
	setOpsEndpointContext(c, "", int16(service.RequestTypeWSV2))
	if publicModel, allowed := groupAllowsClientRequestedModel(c, apiKey.Group, reqModel); !allowed {
		message := groupModelsListDisallowedMessage(publicModel)
	...
	if err := h.billingCacheService.CheckBillingEligibilityFreshSubscription(ctx, apiKey.User, apiKey, apiKey.Group, service.QuotaPlatform(c.Request.Context(), apiKey)); err != nil {
	...
	for {
		...
		if proxyErr != nil {
			if shouldRecordOpenAIWSProxyFailureStat(proxyErr) {
...
func shouldRecordOpenAIWSProxyFailureStat(err error) bool {
	if err == nil {
		return false
	}
	var failoverErr *service.UpstreamFailoverError
	...
	if strings.Contains(err.Error(), "read client websocket request") {
		return false
	}
	return true
}
