...
func TestEffectiveAPIKeyPlatformUsesCompositeRouteDecision(t *testing.T) {
	gin.SetMode(gin.TestMode)
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	c.Request = httptest.NewRequest("POST", "/v1/alpha/search", nil)
	c.Request = c.Request.WithContext(service.WithCompositeRouteDecision(c.Request.Context(), service.CompositeRouteDecision{
		Matched:        true,
		Source:         service.CompositeRouteSourceExplicit,
	...
	require.Equal(t, service.PlatformOpenAI, effectiveAPIKeyPlatform(c, apiKey))
}
