...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	settings := &service.SystemSettings{
		...
		DisplayCurrencySymbol:                  stringSetting(req.DisplayCurrencySymbol, previousSettings.DisplayCurrencySymbol),
		DisplayCurrencySymbolLocalOnly:         boolValueOrDefault(req.DisplayCurrencySymbolLocalOnly, previousSettings.DisplayCurrencySymbolLocalOnly),
...
func (h *SettingHandler) UpdateOpenAIFastPolicySettings(c *gin.Context) {
	var req dto.OpenAIFastPolicySettings
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "Invalid request: "+err.Error())
		return
	}
	if err := h.settingService.SetOpenAIFastPolicySettings(c.Request.Context(), openaiFastPolicySettingsFromDTO(&req)); err != nil {
		response.ErrorFrom(c, err)
		return
	}
	settings, err := h.settingService.GetOpenAIFastPolicySettings(c.Request.Context())
