...
func (h *GatewayHandler) GeminiV1BetaModels(c *gin.Context) {
	...
	setOpsRequestContext(c, clientRequestedModel(c, modelName), stream)
	setOpsEndpointContext(c, "", int16(service.RequestTypeFromLegacy(stream, false)))
	if publicModel, allowed := groupAllowsClientRequestedModel(c, apiKey.Group, modelName); !allowed {
		googleError(c, http.StatusForbidden, groupModelsListDisallowedMessage(publicModel))
		return
	}
