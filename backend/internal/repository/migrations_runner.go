...
var migrationChecksumCompatibilityRules = map[string]migrationChecksumCompatibilityRule{
	...
	"133_allow_email_oauth_provider_types.sql":                newMigrationChecksumCompatibilityRule("21e0bfe39530adea594e7d161d202f2dd201cc8caffeeb8b563ada15ab7f8fc5", "1d5948a0de5fae4fff368e4ae648edb794ce4e7b2704b6e7020c3b881c0f51f3"),
	"135_allow_email_oauth_provider_types.sql":                newMigrationChecksumCompatibilityRule("72b6005ff64cddd845615e249d95858010dd6d3bd7a1d24b3e9a32eb03b76eb8", "e5e3512fd7ff6e9225414bf79425fd8ddbf6d78a66998142bfb2441f8e7e4708"),
...
func applyMigrationsFS(ctx context.Context, db *sql.DB, fsys fs.FS) error {
	...
	defer func() {
		// 无论迁移是否成功，都要释放锁。
		// 独立超时确保原 ctx 取消后仍会尝试释放，但数据库链路异常不会无限阻塞进程退出。
...
func preparePaymentOrdersOutTradeNoUniqueMigration(ctx context.Context, db migrationConnection) error {
	...
	return dropInvalidIndexes(ctx, db, paymentOrdersOutTradeNoUniqueIndex)
}

func dropInvalidIndexes(ctx context.Context, db migrationConnection, indexNames ...string) error {
...
func pgAdvisoryLock(ctx context.Context, db migrationConnection) error {
...
func pgAdvisoryUnlock(ctx context.Context, db migrationConnection) error {
