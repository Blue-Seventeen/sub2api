...
// usageLogSuccessFilterUL excludes failed-request placeholder rows from aggregate statistics.
// There is no success column, so keep every row that has a positive billing or usage signal.
const usageLogSuccessFilterUL = "(" +
	"COALESCE(ul.actual_cost, 0) > 0 OR " +
	"COALESCE(ul.total_cost, 0) > 0 OR " +
	"COALESCE(ul.input_tokens, 0) > 0 OR " +
	"COALESCE(ul.output_tokens, 0) > 0 OR " +
	"COALESCE(ul.cache_creation_tokens, 0) > 0 OR " +
	"COALESCE(ul.cache_read_tokens, 0) > 0 OR " +
	"COALESCE(ul.image_output_tokens, 0) > 0 OR " +
	"COALESCE(ul.image_count, 0) > 0 OR " +
	"COALESCE(ul.request_count, 0) > 0 OR " +
