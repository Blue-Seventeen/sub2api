...
const usageLogSelectColumns = "id, user_id, api_key_id, account_id, request_id, model, requested_model, upstream_model, group_id, subscription_id, input_tokens, output_tokens, cache_creation_tokens, cache_read_tokens, cache_creation_5m_tokens, cache_creation_1h_tokens, image_output_tokens, image_output_cost, input_cost, output_cost, cache_creation_cost, cache_read_cost, total_cost, actual_cost, real_actual_cost, unified_rate_multiplier, rate_multiplier, account_rate_multiplier, billing_type, request_type, stream, openai_ws_mode, duration_ms, first_token_ms, user_agent, ip_address, image_count, image_size, image_input_size, image_output_size, image_size_source, image_size_breakdown, request_count, task_count, billable_duration_seconds, billable_character_count, usage_estimated, billable_unit_type, service_tier, reasoning_effort, inbound_endpoint, upstream_endpoint, client_profile, compatibility_route, fallback_chain, upstream_transport, cache_ttl_overridden, channel_id, model_mapping_chain, billing_tier, billing_mode, account_stats_cost, created_at"
...
var usageLogInsertArgTypes = [...]string{
	...
	"numeric",     // total_cost
	"numeric",     // actual_cost
	"numeric",     // real_actual_cost
	"numeric",     // unified_rate_multiplier
	...
	"integer",     // billable_character_count
	"boolean",     // usage_estimated
	"text",        // billable_unit_type
	"text",        // service_tier
	"text",        // reasoning_effort
	"text",        // inbound_endpoint
	"text",        // upstream_endpoint
	"text",        // client_profile
	"text",        // compatibility_route
	"text",        // fallback_chain
...
const usageLogSuccessFilterUL = `(
	...
	OR COALESCE(ul.billable_duration_seconds, 0) > 0
	OR COALESCE(ul.billable_character_count, 0) > 0
)`
...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	billableUnitType := nullString(log.BillableUnitType)
	serviceTier := nullString(log.ServiceTier)
	reasoningEffort := nullString(log.ReasoningEffort)
	inboundEndpoint := nullString(log.InboundEndpoint)
	upstreamEndpoint := nullString(log.UpstreamEndpoint)
	clientProfile := nullString(log.ClientProfile)
	compatibilityRoute := nullString(log.CompatibilityRoute)
	fallbackChain := nullString(log.FallbackChain)
	upstreamTransport := nullString(log.UpstreamTransport)
...
func (r *usageLogRepository) fillDashboardUsageStatsAggregated(ctx context.Context, stats *DashboardStats, todayUTC, now time.Time) error {
	totalStatsQuery := `
		SELECT
			...
			COALESCE(SUM(real_actual_cost), 0) as real_total_actual_cost,
	...
	todayStatsQuery := `
		SELECT
			...
			real_actual_cost as real_today_actual_cost,
...
func (r *usageLogRepository) fillDashboardUsageStatsFromUsageLogs(ctx context.Context, stats *DashboardStats, startUTC, endUTC, todayUTC, now time.Time) error {
	...
	combinedStatsQuery := `
		...
		SELECT
			...
			COALESCE(SUM(real_actual_cost) FILTER (WHERE created_at >= $1::timestamptz AND created_at < $2::timestamptz), 0) AS real_total_actual_cost,
			...
			COALESCE(SUM(real_actual_cost) FILTER (WHERE created_at >= $3::timestamptz AND created_at < $4::timestamptz), 0) AS real_today_actual_cost,
...
func (r *usageLogRepository) GetUserUsageTrend(ctx context.Context, startTime, endTime time.Time, granularity string, limit int) (results []UserUsageTrendPoint, err error) {
	...
	for rows.Next() {
		...
		if err = rows.Scan(&row.Date, &row.UserID, &row.Email, &row.Username, &row.Requests, &row.Tokens, &row.Cost, &row.ActualCost, &row.RealActualCost); err != nil {
			return nil, err
		}
		results = append(results, row)
	}
...
func (r *usageLogRepository) GetUserSpendingRanking(ctx context.Context, startTime, endTime time.Time, limit int) (result *UserSpendingRankingResponse, err error) {
	...
	query := `
		...
		ranked AS (
			SELECT
				...
				COALESCE(SUM(real_actual_cost) OVER (), 0) as real_total_actual_cost,
				COALESCE(SUM(requests) OVER (), 0) as total_requests,
				COALESCE(SUM(tokens) OVER (), 0) as total_tokens
			FROM user_spend
			ORDER BY real_actual_cost DESC, tokens DESC, user_id ASC
			LIMIT $3
		)
		SELECT
			user_id,
	...
	return &UserSpendingRankingResponse{
		...
		RealTotalActualCost: realTotalActualCost,
...
func (r *usageLogRepository) GetBatchUserUsageStats(ctx context.Context, userIDs []int64, startTime, endTime time.Time) (map[int64]*BatchUserUsageStats, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&userID, &platform, &total, &realTotal, &todayTotal, &realTodayTotal); err != nil {
			_ = rows.Close()
			return nil, err
		}
...
func (r *usageLogRepository) GetBatchAPIKeyUsageStats(ctx context.Context, apiKeyIDs []int64, startTime, endTime time.Time) (map[int64]*BatchAPIKeyUsageStats, error) {
	...
	for rows.Next() {
		...
		var todayTotal float64
		var realTodayTotal float64
		if err := rows.Scan(&apiKeyID, &total, &realTotal, &todayTotal, &realTodayTotal); err != nil {
			_ = rows.Close()
			return nil, err
		}
...
func (r *usageLogRepository) getModelStatsWithFiltersBySource(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, requestType *int16, stream *bool, billingType *int8, source string) (results []ModelStat, err error) {
	...
	realActualCostExpr := "COALESCE(SUM(real_actual_cost), 0) as real_actual_cost"
	...
	query := fmt.Sprintf(`
		...
		FROM usage_logs
		WHERE created_at >= $1 AND created_at < $2
	`, modelExpr, actualCostExpr, realActualCostExpr, accountCostExpr)
...
func (r *usageLogRepository) GetGroupStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, requestType *int16, stream *bool, billingType *int8) (results []usagestats.GroupStat, err error) {
	query := `
		SELECT
			...
			COALESCE(SUM(ul.real_actual_cost), 0) as real_actual_cost,
...
func (r *usageLogRepository) GetUserBreakdownStats(ctx context.Context, startTime, endTime time.Time, dim usagestats.UserBreakdownDimension, limit int) (results []usagestats.UserBreakdownItem, err error) {
	...
	query += " GROUP BY ul.user_id, u.email ORDER BY real_actual_cost DESC"
	if limit > 0 {
...
func (r *usageLogRepository) GetAllGroupUsageSummary(ctx context.Context, todayStart time.Time) ([]usagestats.GroupUsageSummary, error) {
	query := `
		SELECT
			...
			COALESCE(SUM(ul.real_actual_cost), 0) AS real_total_cost,
			COALESCE(SUM(CASE WHEN ul.created_at >= $1 THEN ul.real_actual_cost ELSE 0 END), 0) AS real_today_cost
		FROM groups g
	...
	for rows.Next() {
		...
		if err := rows.Scan(&row.GroupID, &row.TotalCost, &row.TodayCost, &row.RealTotalCost, &row.RealTodayCost); err != nil {
			return nil, err
		}
		results = append(results, row)
	}
...
func (r *usageLogRepository) getEndpointStatsByColumnWithFilters(ctx context.Context, endpointColumn string, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8) (results []EndpointStat, err error) {
	...
	query := fmt.Sprintf(`
		...
		FROM usage_logs
		WHERE created_at >= $1 AND created_at < $2
	`, endpointColumn, actualCostExpr, realActualCostExpr)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&row.Endpoint, &row.Requests, &row.TotalTokens, &row.Cost, &row.ActualCost, &row.RealActualCost); err != nil {
			return nil, err
		}
		results = append(results, row)
	}
...
func (r *usageLogRepository) getEndpointPathStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8) (results []EndpointStat, err error) {
	...
	query := fmt.Sprintf(`
		...
		FROM usage_logs
		WHERE created_at >= $1 AND created_at < $2
	`, actualCostExpr, realActualCostExpr)
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		usageEstimated          bool
		billableUnitType        sql.NullString
		serviceTier             sql.NullString
		reasoningEffort         sql.NullString
		inboundEndpoint         sql.NullString
		upstreamEndpoint        sql.NullString
		clientProfile           sql.NullString
		compatibilityRoute      sql.NullString
		fallbackChain           sql.NullString
		upstreamTransport       sql.NullString
	...
	if breakdown := stringIntMapFromNullJSON(imageSizeBreakdown); len(breakdown) > 0 {
		log.ImageSizeBreakdown = breakdown
	}
	if billableUnitType.Valid {
		log.BillableUnitType = &billableUnitType.String
	}
	...
	if clientProfile.Valid {
		log.ClientProfile = &clientProfile.String
	}
	if compatibilityRoute.Valid {
		log.CompatibilityRoute = &compatibilityRoute.String
	}
	if fallbackChain.Valid {
		log.FallbackChain = &fallbackChain.String
	}
