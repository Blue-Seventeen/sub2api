...
func TestGatewayRoutesAlphaSearchAllowsCompositeRouteToOpenAI(t *testing.T) {
	resolver := service.NewCompositeRouteResolver(compositeRouteRepoStub{
		routes: []service.CompositeModelRoute{
			{
				GroupID:        1,
				PublicModel:    "search-alias",
	...
	req := httptest.NewRequest(http.MethodPost, "/v1/alpha/search", strings.NewReader(`{"model":"search-alias","query":"ping"}`))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()

	router.ServeHTTP(w, req)

	require.NotContains(t, w.Body.String(), "only available for OpenAI groups")
}
