...
func registerContentModerationRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	if h == nil || h.Admin == nil || h.Admin.ContentModeration == nil {
		return
	}
...
func registerProxyRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	proxies := admin.Group("/proxies")
	{
		...
		proxies.GET("/auto-probe/config", h.Admin.Proxy.GetAutoProbeConfig)
		proxies.PUT("/auto-probe/config", h.Admin.Proxy.UpdateAutoProbeConfig)
		proxies.GET("/clash-subscriptions", h.Admin.Proxy.ListClashSubscriptions)
		proxies.POST("/clash-subscriptions", h.Admin.Proxy.CreateClashSubscription)
		proxies.PUT("/clash-subscriptions/:id", h.Admin.Proxy.UpdateClashSubscription)
		proxies.DELETE("/clash-subscriptions/:id", h.Admin.Proxy.DeleteClashSubscription)
		proxies.POST("/clash-subscriptions/:id/refresh", h.Admin.Proxy.RefreshClashSubscription)
		proxies.GET("/clash-subscriptions/:id/status", h.Admin.Proxy.GetClashSubscriptionStatus)
		...
		proxies.GET("/active-usage", h.Admin.Proxy.GetActiveUsage)
		proxies.GET("/snapshots", h.Admin.Proxy.GetSnapshots)
...
func registerSettingsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	adminSettings := admin.Group("/settings")
	{
		...
		adminSettings.GET("/openai-fast-policy", h.Admin.Setting.GetOpenAIFastPolicySettings)
		adminSettings.PUT("/openai-fast-policy", h.Admin.Setting.UpdateOpenAIFastPolicySettings)
		// Web Search 模拟配置
