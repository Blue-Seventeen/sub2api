...
const (
	...
	// AirwallexStaticDomain is the Airwallex production SDK script domain.
	AirwallexStaticDomain = "https://static.airwallex.com"
	// AirwallexCheckoutDomain is the Airwallex production checkout frame domain.
	AirwallexCheckoutDomain = "https://checkout.airwallex.com"
	// AirwallexDemoStaticDomain is the Airwallex demo SDK script domain.
	AirwallexDemoStaticDomain = "https://static-demo.airwallex.com"
	// AirwallexDemoCheckoutDomain is the Airwallex demo checkout frame domain.
	AirwallexDemoCheckoutDomain = "https://checkout-demo.airwallex.com"
	// CSPDataSource allows inline media data URLs for audio previews.
	CSPDataSource = "data:"
	// CSPBlobSource allows Blob URLs generated by the frontend audio preview.
	CSPBlobSource = "blob:"
)
...
func enhanceCSPPolicy(policy string) string {
	...
	// TTS preview audio is rendered from data: or blob: URLs. Older configured
	// policies do not define media-src, so browsers fall back to default-src.
	policy = ensureDirectiveValue(policy, "media-src", CSPDataSource)
	policy = ensureDirectiveValue(policy, "media-src", CSPBlobSource)

	return policy
}

func ensureDirectiveValue(policy, directive, value string) string {
	if directiveContainsValue(policy, directive, value) {
		return policy
	}
