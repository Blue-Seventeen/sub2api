...
const DefaultCSPPolicy = "default-src 'self'; script-src 'self' __CSP_NONCE__ https://challenges.cloudflare.com https://static.cloudflareinsights.com https://*.stripe.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; img-src 'self' data: https:; font-src 'self' data: https://fonts.gstatic.com; connect-src 'self' https:; media-src 'self' data: blob:; frame-src https://challenges.cloudflare.com https://*.stripe.com; frame-ancestors 'none'; base-uri 'self'; form-action 'self'"
...
type GatewayConfig struct {
	...
	// 默认关闭，避免纯文本 Codex 请求被意外改写；显式携带 image_generation 工具的请求仍按现有链路转发。
...
type RateLimitConfig struct {
	...
	AuthRedisFailureMode    string `mapstructure:"auth_redis_failure_mode"`    // fail-close / fail-open
}
...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	if raw := strings.TrimSpace(os.Getenv("SERVER_TRUSTED_PROXIES")); raw != "" {
		cfg.Server.TrustedProxies = splitEnvList(raw)
	}
	...
	cfg.RateLimit.AuthRedisFailureMode = strings.ToLower(strings.TrimSpace(cfg.RateLimit.AuthRedisFailureMode))
	if cfg.RateLimit.AuthRedisFailureMode != "fail-open" {
		cfg.RateLimit.AuthRedisFailureMode = "fail-close"
	}
...
func setDefaults() {
	...
	viper.SetDefault("rate_limit.auth_redis_failure_mode", "fail-close")
